--Group By ---
/*
Group by cumlecigi bir select ifadesinde satirlari, sutunlari degerlerine gore ozet
olarak gruplamak icin kullanilir. 
group by cumlecigi her grup basina satir dondurur.
GROUP BY genelde AVG() COUNT(), max(), min() ve sum() gibi agriggate fonksiyonlari ile birlikte kullanilir
*/
...
--SORU kisi ismine göre satılan toplam meyve miktarlarını gösteren sorguyu yazınız.
...
--Zor soru tekrara bakmalisin (selcuk)
select urun_adi, min(urun_miktari) as minimum_urun_miktari, 
max(urun_miktari) as maximum_urun_miktari from manav 
where urun_adi is not null
group by urun_adi
...
select * from personel;
--Soru personelin calıştığı ülkeleri listeleyiniz

select ulke from personel group by ulke;

--SORU Ülkelere göre ortalama maaşları listeleyiniz
select ulke, round(avg(maas)) as ortalama_maas
from personel group by ulke;

--SORU Maas ortalamasını bayanlar ve baylar olarak sorgulayınız
...
--SORU Her ulke için bay ve bayan çalışan sayısı ve yaş ortalamasını sorgulayınız.
select ulke, cinsiyet, count(*) as calisan_sayisi,avg(yas) as yas_ortalamasi 
from personel group by ulke, cinsiyet;

--SORU Her ulke için bay ve bayan çalışan sayısı ve yaş ortalamasını  
